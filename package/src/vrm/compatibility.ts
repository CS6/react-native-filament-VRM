...
const SUPPORTED_EXTENSIONS = new Set(['VRM', 'VRMC_vrm', 'VRMC_vrm_animation', 'KHR_materials_unlit'])
const FALLBACK_EXTENSIONS = new Set(['KHR_texture_transform', 'KHR_materials_emissive_strength'])
const UNSUPPORTED_VRM_EXTENSIONS = new Set(['VRMC_materials_mtoon', 'VRMC_node_constraint', 'VRMC_springBone'])
...
export function getVRMCompatibilityReport(gltf: VRMGltfJson): VRMCompatibilityReport {
  ...
  if (extensionsUsed.includes('VRMC_node_constraint')) {
    warnings.push('VRMC_node_constraint is not applied yet; constrained secondary nodes may not follow the authored setup.')
  }
