...
function Renderer({
  ...
  React.useEffect(() => {
    ...
    console.log(
      `VRMA retarget ${modelLabel} / ${animationLabel}: ${retargeting.bindings.length} bindings, ${retargeting.expressionBindings.length} expression bindings, ${retargeting.expressionMaterialColorBindings.length} expression material bindings, ${retargeting.lookAtBoneBindings.length} bone lookAt bindings, ${retargeting.lookAtExpressionBindings.length} expression lookAt bindings, ${retargeting.nodeConstraintBindings.length} node constraints, ${retargeting.springBoneBindings.length} spring bones, ${retargeting.clips.length} clips, ${retargeting.clips[0]?.channels.length ?? 0} channels, ${retargeting.clips[0]?.duration ?? 0}s`
    )
    console.log(
